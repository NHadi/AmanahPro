...
func NewConsumerService(esClient *elasticsearch.Client, index, queueName string) *ConsumerService {
	...
	service.handlers["CustomLogicEvent"] = service.handleCustomLogic
...
// handleCustomLogic handles a "CustomLogicEvent"
func (c *ConsumerService) handleCustomLogic(payload map[string]interface{}, meta map[string]interface{}) error {
	log.Printf("Processing custom logic for payload: %v", payload)

	// Example: Perform some custom operation
	if err := customLogicHandler(payload); err != nil {
		return fmt.Errorf("failed to process custom logic: %w", err)
	}

	log.Printf("Successfully processed custom logic for payload")
	return nil
}

// Example: customLogicHandler for business-specific logic
func customLogicHandler(payload map[string]interface{}) error {
	log.Printf("Custom logic executed for payload: %v", payload)
	return nil
}
